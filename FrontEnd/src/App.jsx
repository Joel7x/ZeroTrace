...
function App() {
  ...
  const fetchClaims = async () => {
    try {
      ...
      let fetchedClaims = [];
      try {
        const response = await axios.get('http://localhost:8000/api/v1/claims');
        fetchedClaims = response.data;
      } catch (apiError) {
        console.warn('Backend API not reachable. Falling back to mock data.', apiError);
      ...
      if (fetchedClaims.length > 0 && !selectedClaimId) {
        setSelectedClaimId(fetchedClaims[0].id);
      }
      setError(null);
    } catch (err) {
